...
class gpcrANM:
    ...
    def __init__(self, inactive_pdb, active_pdb, cutoff=15.0, softening_factor=0.1, 
            distance_threshold=2.0, outdir=".", basename="structure"):
        """
        ...
        self.anm = self.build_anm(outdir = self.outdir, basename=self.basename)
    ...
    def build_anm(self, outdir=".", basename="structure"):
        """
        ...
        nmd_path = os.path.join(outdir, f"{basename}_transition.nmd")
        pdb_path = os.path.join(outdir, f"{basename}_inactive.pdb")
        pd.writeNMD(nmd_path, anm[:20], self.inactive_ca)
        pd.writePDB(pdb_path, self.inactive_ca)
        return anm
